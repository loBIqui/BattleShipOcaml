#use "inter.ml" ;;
#use "/home/campussfa/tpatisso/Téléchargements/BattleShipOcaml-main/battleship iter2v2/CPgraphics.ml" ;;
...
let bot_grid : t_coords array =
  [|{x = init_params.margin; y = init_params.margin};
    {x = init_params.margin + (init_params.grid_size * init_params.cell_size); 
     y = init_params.margin + (init_params.grid_size * init_params.cell_size)}|]
;;

let usr_grid : t_coords array =
  [|{x = (init_params.margin * 2) + (init_params.grid_size * init_params.cell_size) ; y = init_params.margin} ;
  {x = (init_params.margin * 2) + (init_params.grid_size * init_params.cell_size * 2) ; y = init_params.margin + (init_params.grid_size * init_params.cell_size)} |]
;;


(**
if x < p_params.margin || x > ((p_params.margin * 2) + (p_params.cell_size * p_params.grid_size * 2)) || (p_params.margin + (p_params.cell_size * p_params.grid_size) < x && x)
*)

let read_mouse (p_params : t_params) : t_coords =
  (x , y) = wait_button_down ;
